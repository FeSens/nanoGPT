...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.think_ahead = config.think_ahead
        self.lm_head = nn.Linear(
            config.n_embd, config.vocab_size * self.think_ahead, bias=False)
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            ...
            loss = F.cross_entropy(
                logits.view(-1, self.think_ahead, logits.size(-1)//self.think_ahead)[:, 0, :], targets[:, :self.config.block_size].reshape(-1), ignore_index=-1)
            tracking_loss.append(loss.item())
            for i in range(1, self.think_ahead):
                l = F.cross_entropy(
                    logits.view(-1, self.think_ahead, logits.size(-1)//self.think_ahead)[:, i, :], targets[:, i:i+self.config.block_size].reshape(-1), ignore_index=-1)
                tracking_loss.append(l.item())
                loss += l/(i)**2

            loss = loss / \
                (1 + sum([1/i**2 for i in range(1, self.think_ahead)]))
            ...
            loss, tracking_loss = None, None

        return logits, loss, tracking_loss
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        flops_promised = 53e11  # A100 GPU bfloat16 peak flops is 312 TFLOPS
